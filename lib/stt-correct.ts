...
const REPLACEMENTS: Array<[RegExp, string]> = [
  ...
  [/\b(llevame|lleveme|mostrame|muestrame|ir a|ir al|a|al)\s+(el |la )?ruta\b/gi, "$1 el RUT"],
  [/\bparte de (la |el )?ruta\b/gi, "sección del RUT"],
  [/\bparte del ruta\b/gi, "parte del RUT"],
  ...
  [/\bmanejo dirico\b/gi, "manejo hídrico"],
  [/\bmanejo idrico\b/gi, "manejo hídrico"],
  [/\bmanejo hidri[ck]o\b/gi, "manejo hídrico"],
  [/\bdirico\b/gi, "hídrico"],
  [/\bidrico\b/gi, "hídrico"],
  [/\btr[aá]mite\s+(a\s+)?(el\s+)?ruth\b/gi, "trámite al RUT"],
  [/\btr[aá]mite\s+(a\s+)?(el\s+)?root\b/gi, "trámite al RUT"],
  [/\ba el rut\b/gi, "al RUT"],
  [/\ba la rut\b/gi, "al RUT"],
  [/\b(llevame|lleveme|mostrame|muestrame|ir a|ir al|a|al)\s+(el\s+)?tr[aá]mite\s+(a\s+)?ruth\b/gi, "$1 el trámite al RUT"],
  [/\b(llevame|lleveme|mostrame|muestrame|ir a|ir al|a|al)\s+(el\s+)?tr[aá]mite\s+(a\s+)?root\b/gi, "$1 el trámite al RUT"],
  [/\brecurso oficial\s+(el\s+)?whats?app\b/gi, "recurso oficial whatsapp"],
  [/\babrir(el|me|)\s+(el\s+)?recurso oficial\s+(el\s+)?whats?app\b/gi, "abrime whatsapp"],
  [/\bprofundicemos en\b/gi, "profundicemos en"],
  [/\bdifundimos en\b/gi, "profundicemos en"],
  [/\bno profundicemos\b/gi, "no profundicemos"],
  [/\bque lo que hace\b/gi, "qué es lo que hace"],
  [/\bque es lo que es\b/gi, "qué es"],
